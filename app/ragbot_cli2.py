...
class RetrieveDocumentsMiddleware(AgentMiddleware[RagState]):
    ...
    def before_model(self, state: RagState, runtime) -> dict[str, Any] | None:
        ...
        state.update({"log_data": log_data})
...
def rag_chain(question, log_data) -> dict[str, str | int | bool | list[Any] | Any]:
    ...
    agent = create_agent(model=llm, tools=[], middleware=[RetrieveDocumentsMiddleware()])
    ...
    for step in agent.stream({
        ...
        log_data["chunks_qty"] = step["log_data"]["chunks_qty"]
        log_data["sources"] = step["log_data"]["sources"]
